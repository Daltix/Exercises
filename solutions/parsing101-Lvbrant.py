...
URLs = ["https://www.gamma.be/nl/assortiment/karcher-hogedrukreiniger-k3-home-rioolset-7-5-m-wasborstel-detergent/p/B545366",
        "https://www.gamma.be/nl/assortiment/cando-buitendeur-cp-brussel-links-215-5x96-cm/p/B258196",
        "https://www.gamma.be/nl/assortiment/cbr-cement-25-kg/p/B300490",
        "https://www.gamma.be/nl/assortiment/brabantia-toiletrolhouder-met-klep-inox-zwart/p/B506099",
        "https://www.gamma.be/nl/assortiment/woood-zetel-louis-3-zit-grijs-melange/p/B557632",
]
...
def extract_product_data(html):
    ...
    #The following are all the html-tags:
    nametag = soup.find("h1", itemprop="name", id="productName").text #kan je miss gwn op h1 tag filteren want is uniek. De text methode haalt hetgeen tussen de html elementen zit
    pricetag_current = str(soup.select("article > div > span > span > div > div > span")[0])
    try:
        pricetag_old = str(soup.select("div > span > span > span > del")[0])
    except IndexError:
        pass

    #Using Regex we capture the necessary information from the tags
    name_re = re.search(r'^(?P<name1>[\w]+) (?P<name2>[\w]+)', nametag)
    price_current_re = re.search(r'(?P<price1>[0-9 \.]+)<.+>(?P<price2>[0-9]+)', pricetag_current)
    try:
        price_old_re = re.search(r'^<del>(?P<price1>[0-9 \.]+)', pricetag_old)
    except UnboundLocalError:
        pass

    #MWith the named capture groups we can now initialize the correct variables:
    product_name = name_re.group("name1")+ " " + name_re.group("name2")
    product_price_new = float(price_current_re.group("price1")+price_current_re.group("price2"))
    try:
        product_price_old = float(price_old_re.group("price1"))
    except UnboundLocalError:
        pass

    #Using the correct variables we can return the desired ProductInfo object:
    try:
        return ProductInfo(product_name,product_price_new,product_price_old)
    except UnboundLocalError:
        return ProductInfo(product_name,product_price_new)
...
def extract_categories(html):
    ...
    soup = bs4.BeautifulSoup(html, "html.parser")
    list_categories_tags= soup.select(".inner > ol > li > a")

    for tag in list_categories_tags:
        url = tag['href']
        category_name = tag.text
        categories.append(Category(url, category_name))

    del categories[0]                 #The "Home" category is removed form the list since it's useless!
    del categories[len(categories)-1] #For some reason the last item in the list was the complete product name
    return categories
